...
var QUADRANT_CHANNELS = {
    "top_left":     1,   // Drums
    "top_right":    2,   // Bass
    "bottom_left":  3,   // Vocals
    "bottom_right": 4    // Other
};
...
function _parseGridPosition(noteNum) {
    // Launchpad Pro Programmer mode: note = row*10 + col
    // Row 1 (bottom) = 11-18, Row 8 (top) = 81-88
    var row = Math.floor(noteNum / 10);  // 1-8
    var col = noteNum % 10;               // 1-8

    if (row < 1 || row > 8 || col < 1 || col > 8) {
        return null;  // not a grid pad (might be a side button)
    }
...
function msg_int(val) {
    ...
    // Determine quadrant and remap note to Drum Rack range
    var quadrant = _getQuadrant(pos.row, pos.col);
    var channel = QUADRANT_CHANNELS[quadrant];
    var drumNote = _remapToDrumRack(pos.row, pos.col);

    // Build output with remapped channel
    var newStatus = (status & 0xF0) | (channel - 1);
